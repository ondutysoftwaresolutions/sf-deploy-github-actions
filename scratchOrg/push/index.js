...
const run = async () => {
    ...
    core_1.info('*** Push changes to Scratch Org ***');
    // push to the org
    const result = execSync_1.default(constants_1.Commands.SFDX, ['force:source:push', '--targetusername', scratchOrgName, '--json']);
    ...
    if (parsedResult.status !== 0) {
        core_1.setFailed(`\u001b[38;2;255;0;0mERROR when pushing the changes: ${parsedResult.name} // ${JSON.stringify(parsedResult)}`);
    }
    else {
