...
exports.installRequirement = void 0;
const core_1 = __nccwpck_require__(186);
const execSync_1 = __importDefault(__nccwpck_require__(205));
const constants_1 = __nccwpck_require__(122);
const Download_1 = __importDefault(__nccwpck_require__(288));
const run = async () => {
    const configuration = new Download_1.default(core_1.getInput('download_sfdx_url'), core_1.getInput('download_sfdx_filename'));
    core_1.info('*** Downloading and installing SFDX ***');
    // download
    execSync_1.default(constants_1.Commands.WGET, [`${configuration.sfdx_url}/${configuration.sfdx_filename}`]);
    core_1.info('*** SFDX Downloaded Successfully ***');
    // create the directory
    execSync_1.default(constants_1.Commands.MKDIR, [constants_1.DEFAULT_SFDX_CLI_INSTALLATION_FOLDER]);
    // decompress the file
    execSync_1.default(constants_1.Commands.TAR, [
        'xJf',
        configuration.sfdx_filename,
        '-C',
        constants_1.DEFAULT_SFDX_CLI_INSTALLATION_FOLDER,
        '--strip-components',
        '1',
    ]);
    core_1.info('*** SFDX Decompressed Successfully ***');
    // run the install
    execSync_1.default('./sfdx-cli/install');
    // add the sfdx to the path
    // execSync(Commands.EXPORT, [`PATH=./${DEFAULT_SFDX_CLI_INSTALLATION_FOLDER}/$(pwd):$PATH`]);
    core_1.info('*** SFDX Installed Successfully ***');
};
...
class Download {
    constructor(url, filename) {
        this.sfdx_url = url || 'https://developer.salesforce.com/media/salesforce-cli';
        this.sfdx_filename = filename || 'sfdx-linux-amd64.tar.xz';
    }
}
