...
const run = async () => {
    const configuration = new Deployment_1.default((0, core_1.getInput)('package_path'), (0, core_1.getInput)('test_level'), (0, core_1.getInput)('wait_time'), (0, core_1.getInput)('deploy'), (0, core_1.getInput)('org_type'), (0, core_1.getInput)('is_destructive'));
    ...
    if (configuration.testLevel) {
        ...
        if (configuration.testLevel === constants_1.TestLevel.LOCAL_TEST) {
            (0, core_1.info)('*** Running All Tests ***');
            params.push(constants_1.TestLevel.LOCAL_TEST);
        }
        else {
            (0, core_1.info)('*** Not Running Tests ***');
...
const exec = (command, params = []) => {
    return new Promise((resolve) => {
        ...
        cmd.on('close', () => {
            const message = errored ? 'ERROR when executing' : 'SUCCESSFUL execution of';
            (0, core_1.info)(`${message} the command ${command} with params ${params.toString()} \n`);
            resolve(result);
        });
    });
};
