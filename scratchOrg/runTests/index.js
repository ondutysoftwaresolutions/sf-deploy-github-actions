...
const run = async () => {
    const minimunTestCoverage = (0, core_1.getInput)('minimum_test_coverage') || constants_1.DEFAULT_MINIMUM_TEST_COVERAGE;
    const scratchOrgName = (0, core_1.getInput)('scratch_org_name');
    (0, core_1.info)('*** Run Tests and check minimun coverage in Scratch Org ***');
    // run the test
    ...
    if (parsedResult.status !== 0) {
        if (parsedResult.name !== 'InvalidAsyncTestJobNoneFound') {
            if (parsedResult.result.summary.failing > 0) {
                (0, core_1.info)('\u001b[38;2;255;0;0m*** Unsuccessful validation of the Package ***\n');
                (0, processValidationResult_1.logTestErrors)(parsedResult.result);
                (0, core_1.setFailed)('The validation of the package failed.');
            }
            else {
                (0, core_1.setFailed)(`\u001b[38;2;255;0;0mERROR when running the tests: ${parsedResult.name} // ${parsedResult.message}`);
            }
        }
        else {
            (0, core_1.info)(`\u001b[35m*** There were no tests to run. Continue with the process ***`);
        }
    }
    else {
        if (parsedResult.result.summary.failing > 0) {
            ...
            if (classesNotMeetingCoverage) {
                ...
                (0, core_1.info)(classesNotMeetingCoverage);
                (0, core_1.setFailed)(`\u001b[38;2;255;0;0mThe minimun test coverage for each class in the project must be equal or higher than ${minimunTestCoverage}.`);
            }
            else {
                (0, core_1.info)(`\u001b[35m*** Tests ran successfully and met the coverage requirements (${minimunTestCoverage}%). ***`);
            }
        }
    }
};
...
const printDeploymentErrorsResult = (errors) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    errors.forEach((err) => {
        (0, core_1.info)(`\u001b[35mName: ${err.fullName}`);
        (0, core_1.info)(`\u001b[35mType: ${err.type}`);
        (0, core_1.info)(`ERROR: ${err.error} \n`);
    });
};
...
const logTestErrors = (result) => {
    ...
    if (numberFailures > 0) {
        (0, core_1.info)(`*** FAILED TESTS (${numberFailures} tests) ***\n`);
...
const processValidationResult = (result) => {
    ...
    if (result.success) {
        (0, core_1.info)(`\u001b[35m*** Successful validation of the Package. JOB ID: ${result.id} ***`);
        (0, core_1.setOutput)('job_id', result.id);
    }
    else {
        (0, core_1.info)('\u001b[38;2;255;0;0m*** Unsuccessful validation of the Package ***\n');
        // if it's test failures
